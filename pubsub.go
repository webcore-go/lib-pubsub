		...
		if config.CredentialsPath != "" {
			...
			if _, err := os.Stat(config.CredentialsPath); os.IsNotExist(err) {
				...
				if config.Credentials != nil {
					...
					logger.Debug("Credentials content", "content", credJSON)
				} else {
			...
			return nil, fmt.Errorf("no credentials provided: either credentials or credentials_data must be specified")
		}
	}
